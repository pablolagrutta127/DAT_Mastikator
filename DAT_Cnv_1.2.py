...
@author: u576890
Pablo La Grutta - Optimización de Acceso Móvil - AEAM
pjlagrutta@teco.com.ar
...
class GUI(tk.Tk):
    ...
    def __init__(self, window): 
        ...
        ##--  lista2 = [x.replace(i,j) if len(x)==7 else x for x in lista for i,j in dictL.items() ] 
    ...
    def Proceso1(self):   
        ...
        print('listaRB',listaRB) #40556,65435,11,-33.0188512802124,-60.6083106994629,4,9360
        ...
        coords_sk = data.as_matrix(columns=['LAT', 'LONG'])
        kms_per_radian = 6371.0088
        epsilon = (self.distB/1000) / kms_per_radian
        print('data',data.head(),data.columns)
        db = DBSCAN(eps=epsilon, min_samples=1, algorithm='ball_tree', metric='haversine').fit(np.radians(coords_sk))
        print('db',db)
        ...
        #df = pd.DataFrame(data={"Resultados": lista})
        #df.to_csv(self.path_res + "/Resultados22_" + ct, sep=';',index=False)



        print(showinfo("Resultados", "Resultados listos!\nCargue nuevos archivos o cierre el programa."))
    ...
    def set_path_lncel(self):
        ...
        print('path_lncel',self.path_lncel)
